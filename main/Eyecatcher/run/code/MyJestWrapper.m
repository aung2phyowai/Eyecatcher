classdef MyJestWrapper < visfield.jest.JestWrapper
    ...
    methods (Access = public)
        ...
        function obj = MyJestWrapper(eye, stimLvlOffset_dB, nFalsePositives, delta_max_cdm2)
...
%             locations_deg  = cat(3, ones(9,1) * 2 * (-21:6:21), (12:-3:-12)' * ones(1,8)) %<-- for testing out-of-bound points
        ...
        function hFig = plotResults(obj, titleStr, fullFn)
            ...
            % validate
            if ~obj.isFinished()
                error('Unable to plot results: test not finished yet??');
            end
