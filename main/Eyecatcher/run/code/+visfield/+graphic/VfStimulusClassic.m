classdef VfStimulusClassic < visfield.graphic.VfStimulus
    ...
    methods (Access = public)
        ...
        function obj = VfStimulusClassic(goldmanType, stim_cycle_on_secs, stim_cycle_off_secs, stim_cycle_n, playSound)
            % VfStimulusClassic Constructor. If playSound is false or omited then no
            ...
            obj.x_audio = audio.rampOnOff(audio.padChannels(audio.getPureTone(200,audio.Fs,d)*lvlFactor, testChans, audio.outChans));
        end
        ...
        function [] = initGraphic(obj, winhandle, stimDiameter_px, warper, shader, back, useStimRamp, useStimWarp, useLegacyMode) % interface implementation
            ...
            % check
            if isempty(obj.stimLuminance_norm)
                error('cannot initialise graphic -- luminance has not been set');
            end
...
%             I_warped = repmat(I_warped,[1 1 2]);
        ...
        function [] = start(obj)
            ...
            if obj.playSound
                blocking = false;
                ivis.audio.IvAudio.getInstance().play(obj.x_audio, [], blocking);
            end
