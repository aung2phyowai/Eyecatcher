classdef ZestNode < handle
	% ########.
    %
    % Public ZestNode Methods:
    ...
    methods (Access = public)
        ...
        function obj = ZestNode(rowId, colId, domain, stopType, stopValue, minStimulus, maxStimulus, minNotSeenLimit, maxSeenLimit, maxPresentations, stimChoice)
            ...
            if ~ismember(obj.stopType, {'npoints', 'stdev', 'entropy'})
                error('ZestNode:Constructor:InvalidInput', 'stopType (%s) must be one of "S", "N", or "H"', obj.stopType);
            end
            if ~ismember(obj.minStimulus, obj.domain)
                error('ZestNode:Constructor:InvalidInput', 'minStimulus (%1.2f) must be included in the specified domain', obj.minStimulus);
            end
            if ~ismember(obj.maxStimulus, obj.domain)
                error('ZestNode:Constructor:InvalidInput', 'maxStimulus (%1.2f) must be included in the specified domain', obj.maxStimulus);
            end
            if obj.minStimulus > obj.maxStimulus
                error('ZestNode:Constructor:InvalidInput', 'specified maxStimulus (%1.2f) must be smaller than specified minStimulus (%1.2f)', obj.maxStimulus, obj.minStimulus);
            end
...
SIGMA = 1.25;                
        ...
        function [] = initialise(obj, startingGuess)
            ...
            if abs(sum(obj.pdf)-1) > 0.000000000001
                error('ZestNode:initialise:InternalError', 'It appears that ZestNode.makePriorPDF did not return a well formed PDF prior??');
            end
		...
		function [] = forceFin(obj)
           	obj.maxPresentations = 0;
        end
        ...
        function stim_dB = getCurrentStimLvl(obj, stimChoice, ensureWithinRange)
            ...
            if ~obj.isInitialised
                error('Node must be initialised used obj.initialise() before it can be used')
            end
        ...
        function isFin = isFinished(obj)
            ...
            % #
            % # Input parameters
            % #   Node list as returned by ZEST.start/step
            % # Returns
            % #   TRUE or FALSE
            % ################################################################################
        ...
        function threshEst_dB = getFinalThresholdEst(obj, stimChoice)
            % ################################################################################
            % # Given a node, return an estimate of threshold
            % #
            % # Input parameters
            % #   Node list as returned by ZEST.start/step
            % # Returns
            % #   Mean   of pdf if node$stimChoice == "mean"
            % #   Mode   of pdf if node$stimChoice == "mode"
            % #   Median of pdf if node$stimChoice == "median"
            % ################################################################################
    ...
    methods (Access = private)
        ...
        function priorPdf = makePriorPDF(obj, startingGuess, weightNormal, pdfFloor)
            if nargin < 3 || isempty(weightNormal)
                weightNormal = 2; % weightNormal>1 biasis towards 'normal'
            end
...
%             healthyPDF = tmp((mode-normalModePoint+obj.domain(1)):(mode-normalModePoint+obj.domain(end)))';
            ...
            glaucomaPDF = normpdf(obj.domain, 0, 1);
            glaucomaPDF = glaucomaPDF ./ sum(glaucomaPDF);
            
            healthyPDF  = normpdf(obj.domain, startingGuess, 1);
            healthyPDF	= healthyPDF ./ sum(healthyPDF);
    ...
    methods (Static, Access = private)
        function sd = stdev(node)
            sd = sqrt(sum(node.pdf .* node.domain .* node.domain) - sum(node.pdf .* node.domain)^2);
        end
        
        function e = entropy(node)
            z = node.pdf>0;
            e = -sum(node.pdf(z) .* log2(node.pdf(z)));
        end
    end
    ...
    methods (Static, Access = public)
        ...
        function [] = runTests()
            ...
            % create example node object
            zs = ZestNode(1, 1, -5:30, 'entropy', 1.5);
