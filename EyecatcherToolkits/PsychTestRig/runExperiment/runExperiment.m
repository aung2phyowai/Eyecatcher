...
% only, but this way ensures that all random number generators are seeded]
 %  - now expect randSeed in config, not randSeedState
    ...
    p.addParameter('select', [], @(x)isPositiveInt(str2num(x)));
    p.addParameter('method', 'inOrder', @(x)any(strcmpi(x,{'randomWithReplacement','randomWithoutReplacement','inOrder'})));
    p.addParameter('from', '', @(x)ischar(x) | iscellstr(x));
    p.addParameter('pid', '', @(x)isPositiveInt(str2num(x)));
    p.addParameter('sid', '', @(x)isPositiveInt(str2num(x)));
    p.addParameter('autoStart', false, @(x)islogical(any2log(x)));
    p.addParameter('skipLoginChecks', false, @(x)islogical(any2log(x)));
    p.addParameter('skipWriteCheck', false, @(x)islogical(any2log(x)));
    p.addParameter('runTimeParams', struct(), @isstruct);
    ...
    skipLoginChecks      	= any2log(p.Results.skipLoginChecks);
    skipWriteCheck       	= any2log(p.Results.skipWriteCheck);
    runTimeParams       	= p.Results.runTimeParams;
    %----------------------------------------------------------------------
...
%  	if isempty(configIDs)
    if strcmp(configIDs,'')
        ...
        if ~isValidConfigIDs(expID,configIDs)
            msg=[   '/*****\n' ...
                'The specified name(s) of one or config file is invalid: {' escape(strjoin1(',',configIDs{:})) '}\n\n' ...
    ...
    try %wrap in try..catch to ensure that we will always properly finish up
        % loginParticipant
      	[partID,partInfo]=loginParticipant(expID, partID, skipLoginChecks); % partID may be empty
        ...
        for i=1:configSelectionNumber
            ...
            if skipWriteCheck
                warning('SKipping write check');
            else
                %functions = depfun(cfg.script,'-toponly', '-quiet');
                functions = matlab.codetools.requiredFilesAndProducts(cfg.script,'toponly');
            ...
            try
                ...
                [varargout{1:nargout(sprintf('%s',script))}] = eval([script '(metaParams,runTimeParams,params{:})']); %!!!!!RUN EXPERIMENT!!!!!
