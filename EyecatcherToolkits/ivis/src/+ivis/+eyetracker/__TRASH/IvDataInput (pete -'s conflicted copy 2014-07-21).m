classdef (Abstract) IvDataInput < Singleton
    ...
    properties (GetAccess = public, SetAccess = private)
        lastXYT_raw = [NaN NaN NaN]; % n.b. pre-processing
        lastXYT_raw_nonNaN = [-inf -inf -inf];
        lastXYT = [NaN NaN NaN];
        lastXYT_nonNaN = [-inf -inf -inf];
    ...
    methods (Access = public)
        ...
        function [] = updateDriftCorrection(~, trueXY, estXY, maxCorrection)
            ...
            ivis.calibration.IvCalibration.getInstance().updateDriftCorrection(trueXY, estXY, maxCorrection);
        end

        function [xy, t] = getLastKnownXY(obj, useRaw, allowNan)
        	% Get the last reported xy gaze coordinate (and also the
        	% timestamp, if requested). Can optionally request the raw
        	% value, rather than the processed (e.g., smoothed,
        	% interpolated) value. Note that if NaNs are not allowed, and
        	% no non-data has yet been received, then xy==[-inf -inf], and
        	% t==-inf.
            %
            % @param    useRaw   optional. Whether to use the unprocessed coordinate (default = false)
            % @param    allowNan optional. Whether to return only the last non-NaN gaze coordinates (default = false)
            % @return   xy       gaze coordinate, in pixels
            % @return   t        timestamp, in seconds
            %
            % @date     26/06/14
            % @author   PRJ
            %  
            ...
            if nargin < 3 || isempty(allowNan)
                allowNan = false;
            end
            
            if allowNan
                if useRaw
                    xy = obj.lastXYT_raw(1:2);
                    t = obj.lastXYT_raw(3);
                else
                    xy = obj.lastXYT(1:2);
                    t = obj.lastXYT(3);
               ...
               if useRaw
                    xy = obj.lastXYT_raw_nonNaN(1:2);
                    t = obj.lastXYT_raw_nonNaN(3);
                else
                    xy = obj.lastXYT_nonNaN(1:2);
                    t = obj.lastXYT_nonNaN(3);
                end 
            end
        end
    end
    ...
    methods(Access = protected)
        ...
        function obj = IvDataInput()
            ...
            % init storage
            obj.buffer1 = nan(obj.p1_Npre,9);
            obj.buffer2 = nan(obj.p2_Npre,9);
            obj.buffer3 = nan(obj.p3_Npre,9);
        ...
        function saccadeOnTime = processData(obj, x, y, t, v, p, logData)
            ...
            % @param    v : some kind of validity measure
            ...
            % new raw data
            newData = [x y t-obj.expectedLatency_sec v p nan(length(x),4)];
            
            % store last known point position and time
            obj.lastXYT_raw = newData(end, 1:3);
            % store non-nan too
            idx = find(all(~isnan(newData(:,1:3)),2),1,'last');
            if ~isempty(idx)
                obj.lastXYT_raw_nonNaN = newData(idx,1:3);
            end
            ...
            %-------------Save data----------------------------------------
            if logData
                ivis.log.IvDataLog.getInstance().add(xy(:,1),xy(:,2),t,vldty,pd,c,d,v,A); % store data in buffer (could precache the singleton to speed things up)
            end
            % store last
            obj.lastXYT = [xy(end,1:2) t(end)];
            % store non-nan too
            idx = find(all(~isnan(xy),2),1,'last');
            if ~isempty(idx)
                obj.lastXYT_nonNaN = [xy(idx,1:2) t(idx)];
            end
    ...
    methods(Access = private)
        ...
        function newDataOut = smoothAndInterpolate(obj, newDataIn)
            ...
            m = obj.p1_ww2;
            for i = idx
                smoothed = mean(tmp((i-m):(i+m),1),1);
                obj.buffer1(i,~isnan(smoothed)) = smoothed;
            end
        ...
        function newDataOut = tagEvents(obj, newDataIn)
            ...
            % identify saccades
            d = [NaN; sqrt(sum(diff(obj.buffer2(:,1:2),1).^2,2))]; % calc euchlidean distance in points
            d = d / obj.pixel_per_dg; % ivis.math.IvUnitHandler.getInstance().px2deg(d);% convert to degrees visual angle
            dt = [NaN; diff(obj.buffer2(:,3))]; % time diff in seconds
            v = d./dt; % calc velocity (deg/sec)
            A = [NaN; diff(v)]; % calc acceleration
            distmetric = sqrt(sum((obj.buffer2(idx-obj.p2_dOffsetPrev,1:2)-obj.buffer2(idx+obj.p2_dOffsetPost,1:2)).^2,2));
            distmetric = distmetric / obj.pixel_per_dg; % ivis.math.IvUnitHandler.getInstance().px2deg(distmetric);% convert to degrees visual angle
            ...
            isSaccadeOn = (A(idx) > obj.accelCriterion & v(idx) > obj.velocityCriterion & distmetric > obj.distanceCriterion);
