classdef (Abstract) IvClassifier < ivis.broadcaster.IvListener
    ...
    properties (GetAccess = public, SetAccess = protected)
        ...
        % New!
        velocity_exclusion_criterion_degsec = 5;
    end
    ...
    methods(Access = protected)
        ...
        function xyt = getData(obj)
            ...
            xytv = ivis.log.IvDataLog.getInstance().getTmpBuff([1 2 3 8]); % n.b., this is post-processing, so will include calibration and drift-correction, etc
            
            % ALT(?):
            % xyt = ivis.log.IvDataLog.getInstance().getSinceT(obj.lastEvaluatedTimepoint, 1:3);
            % ^^ [tbh this seems like a perfectly good/straightforward
            % solution, rather than the above, and that way the tmpBuff
            % could be left intact for users to query themselves(?)]
            
            % keep only those points that fall below a specific velocity
            % criterion
            if ~isempty(xytv)
                % fprintf('%i of %i points removed based on velocity\n', sum(xytv(:,4)>obj.velocity_exclusion_criterion_degsec), size(xytv,1));
                xyt = xytv(xytv(:,4)<obj.velocity_exclusion_criterion_degsec, 1:3);
            else
                xyt = [];
                return;
            end
